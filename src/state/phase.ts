...
// Social/growth-focused checklists tailored to project type
// These are about getting users and traction, not technical polish
const PROJECT_CHECKLISTS: Record<ProjectType, GraduationItem[]> = {
  'cli': [
    { key: 'ANNOUNCE', name: 'Announce', action: 'Post to r/commandline, r/programming, HN', detail: 'CLI users hang out in tech communities - show a demo GIF' },
    { key: 'GIF', name: 'Demo', action: 'Create a terminal GIF showing the tool in action', detail: 'asciinema or terminalizer - motion sells CLI tools' },
    { key: 'NETWORK', name: 'Network', action: 'DM 10 devs who use similar tools', detail: 'Find people complaining about alternatives on Twitter/Reddit' },
    { key: 'FEEDBACK', name: 'Feedback', action: 'Ask 5 users: what flag is missing?', detail: 'CLI power users have strong opinions - listen to them' },
    { key: 'CONTENT', name: 'Content', action: 'Write "I built a CLI that does X" post', detail: 'Dev.to, Hashnode, your blog - developers read these' },
    { key: 'COMPARE', name: 'Compare', action: 'Create honest comparison with alternatives', detail: 'Help users decide - honesty builds trust' },
    { key: 'MEASURE', name: 'Measure', action: 'Track npm downloads weekly', detail: 'npm-stat.com or similar - know if growth is working' },
    { key: 'ITERATE', name: 'Iterate', action: 'Ship most-requested feature within a week', detail: 'Fast iteration signals active maintenance' },
  ],
  'library': [
    { key: 'ANNOUNCE', name: 'Announce', action: 'Post to framework-specific Discord/Reddit', detail: 'React libs go to r/reactjs, Vue to r/vuejs, etc.' },
    { key: 'EXAMPLE', name: 'Example', action: 'Create a CodeSandbox/StackBlitz demo', detail: 'Tryable demos convert better than docs' },
    { key: 'NETWORK', name: 'Network', action: 'DM 10 devs building similar apps', detail: 'Find people asking "how do I do X" - your lib might help' },
    { key: 'FEEDBACK', name: 'Feedback', action: 'Ask 5 users: what API is confusing?', detail: 'DX feedback improves adoption rates' },
    { key: 'CONTENT', name: 'Content', action: 'Write tutorial: "How to do X with [your lib]"', detail: 'Tutorials rank in Google and attract users' },
    { key: 'COMPARE', name: 'Compare', action: 'Document when NOT to use your library', detail: 'Honest scoping builds trust and reduces bad reviews' },
    { key: 'MEASURE', name: 'Measure', action: 'Track weekly npm downloads', detail: 'Set a goal: 100/week, then 1000/week' },
    { key: 'ITERATE', name: 'Iterate', action: 'Fix top GitHub issue within a week', detail: 'Responsive maintenance attracts contributors' },
  ],
  'web-app': [
    { key: 'LAUNCH', name: 'Launch', action: 'Post to Product Hunt, IndieHackers, HN', detail: 'Coordinate launch day - post early morning PT' },
    { key: 'VIDEO', name: 'Video', action: 'Create 60-second demo video', detail: 'Loom or screen recording - show the happy path' },
    { key: 'NETWORK', name: 'Network', action: 'DM 20 people in your target market', detail: 'Personal outreach converts 10x better than posts' },
    { key: 'FEEDBACK', name: 'Feedback', action: 'Ask 5 users to share screen while using it', detail: 'Watch confusion happen - it reveals UX issues' },
    { key: 'PROOF', name: 'Proof', action: 'Get 3 testimonials with photos', detail: 'Real faces + quotes on landing page = trust' },
    { key: 'CONTENT', name: 'Content', action: 'Write "How I built X in Y days" post', detail: 'Build in public content attracts early adopters' },
    { key: 'MEASURE', name: 'Measure', action: 'Set up one key metric dashboard', detail: 'Signups, active users, or revenue - pick one to obsess over' },
    { key: 'ITERATE', name: 'Iterate', action: 'Ship top user request this week', detail: 'Show users you listen - builds loyalty fast' },
  ],
  'api': [
    { key: 'ANNOUNCE', name: 'Announce', action: 'Post to API-focused communities', detail: 'Dev.to, relevant Discord servers, Twitter API community' },
    { key: 'EXAMPLE', name: 'Example', action: 'Create copy-paste code snippets', detail: 'curl + JS + Python examples - reduce friction to first call' },
    { key: 'NETWORK', name: 'Network', action: 'DM 10 devs building in your space', detail: 'Find people asking "is there an API for X?"' },
    { key: 'FEEDBACK', name: 'Feedback', action: 'Ask 5 integrators: what was confusing?', detail: 'First integration experience determines word of mouth' },
    { key: 'CONTENT', name: 'Content', action: 'Write "Build X with our API" tutorial', detail: 'End-to-end tutorials attract developers searching for solutions' },
    { key: 'PROOF', name: 'Proof', action: 'Get 3 "Powered by" logos', detail: 'Show who uses your API - social proof for B2B' },
    { key: 'MEASURE', name: 'Measure', action: 'Track weekly API calls and unique users', detail: 'Usage metrics reveal if growth tactics work' },
    { key: 'ITERATE', name: 'Iterate', action: 'Add most-requested endpoint this week', detail: 'Fast iteration attracts serious integrators' },
  ],
  'mobile': [
    { key: 'LAUNCH', name: 'Launch', action: 'Coordinate App Store + social launch', detail: 'Same day: store live + Product Hunt + Twitter thread' },
    { key: 'VIDEO', name: 'Video', action: 'Create app preview video', detail: 'Show the happy path in 30 seconds - vertical format' },
    { key: 'NETWORK', name: 'Network', action: 'DM 20 people who need this app', detail: 'Find people complaining about the problem you solve' },
    { key: 'FEEDBACK', name: 'Feedback', action: 'Watch 5 people use the app', detail: 'In person or screen share - watch where they get stuck' },
    { key: 'REVIEWS', name: 'Reviews', action: 'Ask happy users for App Store reviews', detail: 'Time the ask after a success moment in the app' },
    { key: 'CONTENT', name: 'Content', action: 'Post app journey on Twitter/TikTok', detail: 'Build in public attracts early adopters' },
    { key: 'MEASURE', name: 'Measure', action: 'Track daily active users', detail: 'DAU is the metric that matters for mobile' },
    { key: 'ITERATE', name: 'Iterate', action: 'Ship update within 2 weeks of launch', detail: 'Active updates improve store ranking' },
  ],
  'monorepo': [
    { key: 'ANNOUNCE', name: 'Announce', action: 'Announce each package to its audience', detail: 'Different packages may have different communities' },
    { key: 'EXAMPLE', name: 'Example', action: 'Create example app using multiple packages', detail: 'Show how packages work together' },
    { key: 'NETWORK', name: 'Network', action: 'DM maintainers of related projects', detail: 'Collaboration opportunities and cross-promotion' },
    { key: 'FEEDBACK', name: 'Feedback', action: 'Ask contributors: what blocked you?', detail: 'Monorepos are complex - reduce friction' },
    { key: 'CONTENT', name: 'Content', action: 'Write "Why we chose monorepo" post', detail: 'Architecture posts attract serious developers' },
    { key: 'CONTRIB', name: 'Contributors', action: 'Respond to first PRs within 24 hours', detail: 'Fast response = more contributors' },
    { key: 'MEASURE', name: 'Measure', action: 'Track downloads per package', detail: 'Know which packages drive adoption' },
    { key: 'ITERATE', name: 'Iterate', action: 'Fix cross-package issue this week', detail: 'Show the monorepo actually works together' },
  ],
  'unknown': [
    { key: 'ANNOUNCE', name: 'Announce', action: 'Post to 3 communities where users hang out', detail: 'Reddit, Discord, Twitter, Hacker News - find your people' },
    { key: 'NETWORK', name: 'Network', action: 'DM 10 people who would find this useful', detail: 'Personal outreach converts 10x better than posts' },
    { key: 'FEEDBACK', name: 'Feedback', action: 'Ask 5 users: what\'s confusing? what\'s missing?', detail: 'Real users reveal blind spots you can\'t see' },
    { key: 'PROOF', name: 'Proof', action: 'Get 3 testimonials, screenshot your metrics', detail: 'Social proof compounds - collect it early' },
    { key: 'ITERATE', name: 'Iterate', action: 'Ship one improvement based on feedback', detail: 'Show users you listen - builds loyalty fast' },
    { key: 'CONTENT', name: 'Content', action: 'Write "what I learned building X" post', detail: 'Teaching builds authority and attracts users' },
    { key: 'MEASURE', name: 'Measure', action: 'Set up analytics for your key metric', detail: 'Users, downloads, revenue - pick one to obsess over' },
    { key: 'AUTOMATE', name: 'Automate', action: 'Set up one recurring growth loop', detail: 'Newsletter, social schedule, referral system' },
  ],
};
